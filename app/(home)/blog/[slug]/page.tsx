  ...
  return (
    <article className="container py-6 prose dark:prose-invert max-w-3xl mx-auto">
      ...
      <div className="flex gap-2 mb-2 flex-wrap">
        {post.tags?.map((tag) => <Tag tag={tag} key={tag.id} />)}
      </div>
      {post.summary ? (
      ...
      <Markdown
        ...
        className="whitespace-normal break-words overflow-x-hidden"
        components={{
          ...
          link: ({ children }) => (
            <Link href={children as string} target="_blank" rel="noreferrer">
              {children}
            </Link>
          ),
        }}
